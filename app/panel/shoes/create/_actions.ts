...
async function handleCreateCategorySubmit(state: m.InitialStateEntries, formData: FormData): Promise<m.InitialStateEntries> {
  try {
    ...
    const result = await createCategoryValdiationSchema.validate(payload, { abortEarly: false });
