...
const createUserSchema = z.object({
  name: z.string().min(3, "O nome deve ter pelo menos 3 caracteres.").max(100, "O nome pode ter no máximo 100 caracteres.").nonempty("O nome é obrigatório."),
  ...
  password: z.string().min(6, "A senha deve ter pelo menos 6 caracteres.").max(50, "A senha pode ter no máximo 50 caracteres.").nonempty("A senha é obrigatória."),
});
...
const updateUserSchema = createUserSchema.extend({
  ...
  password: z.string().min(6, "A senha deve ter pelo menos 6 caracteres.").max(50, "A senha pode ter no máximo 50 caracteres.").optional().default("").or(z.literal("")),
});

export { loginSchema, createUserSchema, updateUserSchema, getUsersPaginatedSchema };
