...
const UsersActiveChart: React.FC<UsersActiveChartProps> = ({ initialData }) => {
  ...
  const handleDateChange = async (startDate: string, endDate: string) => {
    ...
    try {
      const result = await getUsersActiveByDateAction(startDate, endDate);
...
export default React.memo(UsersActiveChart);
